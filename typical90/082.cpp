...
int main() {
  ...
  num ans = 0;
  ll p = 1;
  for (int l = 1; l <= 18; l++, p *= 10) {
    ll left = p, right = p * 10 - 1;
    assert(right <= LLONG_MAX);
    if (left <= R && R <= right) {
      ans += num(max(left, L) + R) * num(R - max(left, L) + 1) / 2 * l;
    }
    if (right < R && L <= right) {
      ans += num(max(left, L) + right) * num(right - max(left, L) + 1) / 2 * l;
    }
  }
  if (R == ll(1e18)) {
    ans += num(R) * 19;
