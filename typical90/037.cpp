...
ll e() { return -1; }
ll mapping(ll f, ll g) { return max(f, g); }
ll composition(ll f1, ll f2) { return max(f1, f2); }
ll id() { return -1; }
int main() {
  ...
  int W, N;
  cin >> W >> N;
  using segtree_t = atcoder::lazy_segtree<ll, op, e, ll, mapping, composition, id>;
  vector<segtree_t> ST(2, segtree_t(W + 1));
  int cur = 0;
  ST[cur].set(0, 0);
  for (int i = 0; i < N; i++) {
    int L, R, V;
    ...
    ST[cur ^ 1] = ST[cur];
    for (int w = 0; w < W; w++) {
      ll pval = ST[cur].get(w);
      if (pval != -1) {
        int l = w + L;
        int r = w + R;
        if (l <= W) {
          r = min(r, W);
          ST[cur ^ 1].apply(l, r + 1, pval + V);
        }
      }
    }
    cur ^= 1;
  }
  cout << ST[cur].get(W) << endl;
}