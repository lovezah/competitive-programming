...
int e() { return 0; }
int mapping(int f, int g) { return max(f, g); }
int composition(int f1, int f2) { return max(f1, f2); }
int id() { return 0; }
int main() {
  ...
  int W, N;
  cin >> W >> N;
  atcoder::lazy_segtree<int, op, e, int, mapping, composition, id> ST(W);
  for (int i = 0; i < N; i++) {
    int L, R;
    cin >> L >> R;
    L--;
    int cur_h = ST.prod(L, R);
    ST.apply(L, R, cur_h + 1);
    cout << cur_h + 1 << nl;
  }
}